...
class Gingivalindexdata extends ChangeNotifier {
  int ging_scorepl = 0;
  ...
  // Map to store gingival values for each tooth surface.
  // Key format: "ToothNumberSurface" (e.g., "17B", "21P", "47L")
  final Map<String, String> gingivalValues = {};

  Gingivalindexdata() {
    initializegingivalValues();
  }

  // Initializes all plaque values to empty strings.
  // In Gingivalindexdata class
  void initializegingivalValues() {
    for (String tooth in upperTeeth) {
      gingivalValues['${tooth}B1'] = '0'; // Buccal/Labial 1
      gingivalValues['${tooth}B2'] = '0'; // Buccal/Labial 2
      gingivalValues['${tooth}B3'] = '0'; // Buccal/Labial 3
      gingivalValues['${tooth}P'] = '0'; // Palatal
    }
    for (String tooth in lowerTeeth) {
      gingivalValues['${tooth}B1'] = '0'; // Buccal/Labial 1
      gingivalValues['${tooth}B2'] = '0'; // Buccal/Labial 2
      gingivalValues['${tooth}B3'] = '0'; // Buccal/Labial 3
      gingivalValues['${tooth}P'] = '0'; // Lingual
    }
  }

  // Gets the gingival value for a specific tooth and surface.
  String getgingivalValue(String toothNum, String surface) {
    return gingivalValues['$toothNum$surface'] ?? '';
  }

  // Sets the gingival value for a specific tooth and surface.
  void setgingivalValue(String toothNum, String surface, String value) {
    if (value == '0' ||
        value == '1' ||
        value == '2' ||
        value == '3' ||
        value.isEmpty) {
      gingivalValues['$toothNum$surface'] = value;
  ...
  try {
    ...
    if (data['individualScores'] is Map) {
      ...
      individualScores.forEach((key, value) {
        if (gingivalValues.containsKey(key) && value is String) {
          gingivalValues[key] = value;
        }
      });
    }
  ...
  // Calculates the total gingival score based on entered '1's.
  int calculateScore() {
    int score = 0;
    gingivalValues.forEach((key, value) {
      if (value != '0' && value != '') {
